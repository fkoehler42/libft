/* ************************************************************************** */
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   print_hexa.c                                       :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: fkoehler <fkoehler@student.42.fr>          +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2016/02/08 12:03:36 by fkoehler          #+#    #+#             */
/*   Updated: 2016/02/08 15:09:48 by fkoehler         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "ft_printf.h"

void	print_hexa(unsigned long long nb, t_arg *arg)
{
	int	nb_char;

	nb_char = count_char_usigned_decimal_base(nb, 16);
	if (arg->letter == 'X')
		print_hexa_upper(nb, nb_char, arg);
	else
	{
		if (arg->sharp && nb != 0)
		{
			g_bytes += 2;
			arg->width -= 2;
			if (arg->minus || arg->zero)
				ft_putstr("0x");
		}
		if (arg->minus)
			minus_hexa(nb, nb_char, arg);
		else if (arg->zero)
			zero_hexa(nb, nb_char, arg);
		else
		{
			if (nb == 0 && arg->precision == 0)
				arg->width++;
			no_flag_hexa(nb, nb_char, arg);
		}
	}
}

void	minus_hexa(unsigned long long nb, int nb_char, t_arg *arg)
{
	if (arg->precision > nb_char)
	{
		ft_putnzeros(arg->precision - nb_char);
		g_bytes += arg->precision - nb_char;
	}
	if (nb == 0 && arg->precision == 0 && ++g_bytes)
		ft_putchar(' ');
	else if (g_bytes += nb_char)
		ft_put_ullong_base(nb, 16);
	if (arg->width > arg->precision && arg->width > nb_char)
	{
		if (arg->precision > nb_char)
		{
			ft_putnspaces(arg->width - arg->precision);
			g_bytes += arg->width - arg->precision;
		}
		else if (nb_char >= arg->precision)
		{
			ft_putnspaces(arg->width - nb_char);
			g_bytes += arg->width - nb_char;
		}
	}
}

void	zero_hexa(unsigned long long nb, int nb_char, t_arg *arg)
{
	if (arg->width > nb_char && arg->precision == -1)
	{
		ft_putnzeros(arg->width - nb_char);
		ft_put_ullong_base(nb, 16);
		g_bytes += arg->width;
	}
	else
	{
		if (nb == 0 && arg->precision == 0)
			arg->width++;
		no_flag_hexa(nb, nb_char, arg);
	}
}

void	no_flag_hexa(unsigned long long nb, int nb_char, t_arg *arg)
{
	if (arg->width > arg->precision && arg->width > nb_char)
	{
		if (arg->precision > nb_char)
		{
			ft_putnspaces(arg->width - arg->precision);
			g_bytes += arg->width - arg->precision;
		}
		else if (nb_char >= arg->precision)
		{
			ft_putnspaces(arg->width - nb_char);
			g_bytes += arg->width - nb_char;
		}
	}
	if (!arg->zero && arg->sharp && nb != 0)
		ft_putstr("0x");
	if (arg->precision > nb_char)
	{
		ft_putnzeros(arg->precision - nb_char);
		g_bytes += arg->precision - nb_char;
	}
	if (!(nb == 0 && arg->precision == 0))
	{
		ft_put_ullong_base(nb, 16);
		g_bytes += nb_char;
	}
}
